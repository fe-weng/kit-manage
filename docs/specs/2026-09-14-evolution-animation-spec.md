...
`EvolutionFx.tsx` 里的 `SHELL_SHARD_IMAGES` 写明每种宠物用哪三张图。名册里有的宠物，壳片只在 0.36s 的闪光峰值出现并立即飞出，不提前盖在蛋上。没有登记的种类不画几何色块，只保留裂纹、闪光和粒子。

| 宠物 | 状态 | 文件 |
|------|------|------|
| 小兔 | 已接入 | `public/pets/rabbit/evo-shell-left.png` / `right` / `top` |
| 小鸡 | 已接入 | `public/pets/chicken/evo-shell-left.png` / `right` / `top` |
